...
run_fixall() {
    ...
    if bash "$HOME/MyGensynNodeSetup/fixall.sh" >/dev/null 2>&1; then
...
main_menu() {
    while true; do
        ...
        echo ""
        echo -e "   ${BOLD}[1]${NC} Setup / Update Node"
        echo -e "   ${BOLD}[2]${NC} Start Node"
        echo -e "   ${BOLD}[3]${NC} Change Settings"
        echo -e "   ${BOLD}[4]${NC} Generate New Peer ID"
        echo -e "   ${BOLD}[5]${NC} Wipe & Reset"
        echo -e "   ${BOLD}[6]${NC} Install Older Version"
        echo -e "   ${BOLD}[7]${NC} Exit"
        echo ""
        
        read -p "${BOLD}${CYAN}➡️ Enter your choice [1-7]: ${NC}" choice
        ...
        case $choice in
            ...
            7)
                echo -e "\n${GREEN}✅ Exiting... Thank you for using Gensyn Auto-Pilot!${NC}"
                exit 0
                ;;
            *)
